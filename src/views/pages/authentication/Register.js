			...
			await useJwt
				...
				.then((res) => {
					if (res.data.status) {
						...
						toast.success(<ToastContentValid name={`${data.firstName} ${data.lastName}` || 'John Doe'} role={data.role || 'admin'} />, {
							transition: Slide,
					...
					<Col className="px-xl-2 mx-auto" sm="8" md="6" lg="12">
						...
						<AvForm className="auth-register-form mt-2" onSubmit={handleSubmit}>
							...
							<FormGroup>
								...
								/>
								{Object.keys(errors).length && errors.email ? <small className="text-danger">{errors.email}</small> : null}
							</FormGroup>
							<FormGroup>
